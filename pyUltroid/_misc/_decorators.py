    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                ftext = "**Rizmil Client Error:** `Forward this to` @kunthulsupport\n\n"
                ftext += "**Py Version:** `" + str(pyver)
                ftext += "`\n**Rizmil Version:** `" + str(ult_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START RIZMIL CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Rizmil Client Error:** `Forward this to` @kunthulsupport\n\n",
                        )
                else:
