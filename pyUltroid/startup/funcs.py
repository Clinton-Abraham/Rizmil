...
async def autobot():
    ...
    if who.username:
        ...
        username = "rizmil_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "rizmil_" + (str(who.id))[6:] + str(ran) + "_bot"
        await ultroid_bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await ultroid_bot.get_messages(bf, limit=1))[0].text
...
async def autopilot():
    ...
    if not channel:
        ...
        try:
            r = await ultroid_bot(
                CreateChannelRequest(
                    title="Rizmil Userbot Log",
                    about="Rizmil Userbot Log -> Join @kunthulsupport",
                    megagroup=True,
                ),
            )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://graph.org/file/e99dc35f50e69851093fd.jpg", "channelphoto.jpg"
        )
...
async def customize():
    ...
    try:
        ...
        file = random.choice(
            [
                "https://graph.org/file/e99dc35f50e69851093fd.jpg",
                "https://graph.org/file/e99dc35f50e69851093fd.jpg",
        ...
        await ultroid_bot.send_message(
            "botfather",
            f"✨ Powerful Rizmil Userbot Assistant ✨\n✨ Master ~ {sir} ✨",
        )
        await asyncio.sleep(2)
        ...
        try:
            async for x in ultroid_bot.iter_messages(
                ...
                try:
                    ...
                    LOGS.info(f"RIZMIL - PLUGIN_CHANNEL - ERROR - {plugin}")
                    LOGS.exception(e)
                    os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """**Thanks for Deploying Rizmil Userbot !**
❀ Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://graph.org/file/e99dc35f50e69851093fd.jpg"
        BTTS = Button.inline("❀ Click to Start ❀", "initft_2")
        udB.set_key("INIT_DEPLOY", "Done")
    else:
        MSG = f"**Rizmil Userbot has been deployed!**\n➖➖➖➖➖➖➖➖➖➖\n**UserMode**: {inline_mention(ultroid_bot.me)}\n**Assistant**: @{asst.me.username}\n➖➖➖➖➖➖➖➖➖➖"
